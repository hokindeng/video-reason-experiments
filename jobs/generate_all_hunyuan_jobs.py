...
def generate_job_scripts():
    ...
    for task_dir in task_dirs:
        ...
        slurm_file = JOBS_DIR / f"hunyuan_{task_short}.slurm"
...
def generate_master_submit_script(job_files):
    ...
    submit_script = JOBS_DIR / "submit_all_hunyuan.sh"
...
JOB_ID=$(sbatch --parsable "$SCRIPT_DIR/{slurm_filename}")
JOB_IDS+=($JOB_ID)
echo "  Job ID: $JOB_ID"
"""
...
def generate_individual_submit_scripts(job_files):
    ...
    for job_info in job_files:
        ...
        submit_script = JOBS_DIR / f"submit_hunyuan_{task_short}.sh"
        slurm_filename = job_info['slurm_file'].name
...
JOB_FILE="$SCRIPT_DIR/{slurm_filename}"
...
def main():
    ...
    logs_dir = PROJECT_ROOT / "video-reason-experiments" / "logs"
    ...
    print(f"     ./jobs/submit_all_hunyuan.sh")
    print()
    print("  2. Submit individual jobs:")
    print(f"     ./jobs/submit_hunyuan_G1.sh")
    print(f"     ./jobs/submit_hunyuan_G7.sh")
    print("     ... etc")
    print()
    print("  3. Submit range of jobs:")
    print("     for i in {{1..10}}; do ./jobs/submit_hunyuan_G$i.sh; done")
    print()
