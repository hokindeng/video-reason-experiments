...
**Fix in script**: Already handled by explicit path finding in `slurm/hunyuan-video-i2v/setup/install_flash_attention.slurm`
...
**If still failing**: Edit `slurm/hunyuan-video-i2v/setup/install_flash_attention.slurm`:
```bash
...
**Solution**: Already fixed - `slurm/hunyuan-video-i2v/setup/setup_environment.sh` uses cluster PyTorch 2.7+cu126 with `--system-site-packages`
...
- See [../slurm/hunyuan-video-i2v/docs/FLASH_ATTENTION.md](../slurm/hunyuan-video-i2v/docs/FLASH_ATTENTION.md) for flash-attention specifics
...
**Location**: `slurm/hunyuan-video-i2v/setup/setup_environment.sh`
...
4. ✅ Run `./slurm/common/check_setup.sh`
